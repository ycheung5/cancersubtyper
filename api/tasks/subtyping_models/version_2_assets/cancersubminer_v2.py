...
SAMPLE_DIR = Path(settings.sample_file)
if not SAMPLE_DIR.is_dir():
    SAMPLE_DIR = SAMPLE_DIR.parent
MODEL_SAMPLE_DIR = SAMPLE_DIR / "cancersubminer"
...
def preprocess_cancersubminer(
        ...
        source_file: str,
        target_file: str,
        model_parameters: list # ADDED BY SARAH
):
    parameters = normalize_model_parameters("CancerSubminer", model_parameters)
    is_using_pretrained_model = parameters["is_using_pretrained_model"]
    num_cpg_clusters = parameters["num_cpg_clusters"]
    ...
    if is_using_pretrained_model == 0 :
        ...
        X_source = np.load(ASSET_DIR / "X_source.npy")
        y_source = np.load(ASSET_DIR / "y_source.npy")
        sample_ids = np.load(ASSET_DIR / "sample_ids.npy", allow_pickle=True)

        raw_x = pd.DataFrame(X_source, index=sample_ids)
        raw_y = pd.DataFrame(y_source, columns=["subtype"], index=sample_ids)
        ...
        cpg_cluster_info = cpg_info_table[["CpG", "cluster"]].rename(columns={"CpG": "cpg"})
        cpg_cluster_info.to_csv(os.path.join(preprocessing_path, "cpg_cluster_info.csv"), mode="w", index=False)
...
def run_cancersubminer(
    ...
    parameters = normalize_model_parameters("CancerSubminer", model_parameters)
    is_automatically_estimation_required = parameters["is_automatically_estimation_required"]
    num_subtype_user_defined = parameters["num_subtype_user_defined"]
    is_using_pretrained_model = parameters["is_using_pretrained_model"]
    pretrained_ckpt_path = ASSET_DIR / "cancersubminer_full_pretrained.pt"
    ...
    if is_using_pretrained_model == 1 :
        X_source = np.load(ASSET_DIR / "X_source.npy")
        y_source = np.load(ASSET_DIR / "y_source.npy")
        sample_ids = np.load(ASSET_DIR / "sample_ids.npy", allow_pickle=True)
        raw_x = pd.DataFrame(X_source)
        raw_y = pd.DataFrame(y_source)
        raw_y.columns =['subtype']
    ...
    target_batch_size = parameters["target_batch_size"]
    ...
    n_fe_embed1 = parameters["num_hidden_nodes1_feature_extractor"]
    n_fe_embed2 = parameters["num_hidden_nodes2_feature_extractor"]
    n_c_h1 = parameters["num_hidden_nodes1_classifier"]
    n_c_h2 = parameters["num_hidden_nodes2_classifier"]
    n_d_h1 = parameters["num_hidden_nodes1_discriminator"]
    n_d_h2 = parameters["num_hidden_nodes2_discriminator"]
    ...
    fe_optimizer = torch.optim.Adam(feature_extract_model.parameters(), lr=parameters["learning_rate_feature_extractor"])
    c_optimizer = torch.optim.Adam(subtype_pred_model.parameters(), lr=parameters["learning_rate_classifier"])
    d_optimizer = torch.optim.Adam(domain_disc_model.parameters(), lr=parameters["learning_rate_discriminator"])

    # === load pretrained if requested ===
    if is_using_pretrained_model == 1 :
        ckpt = torch.load(str(pretrained_ckpt_path), map_location=device)
        feature_extract_model.load_state_dict(ckpt["feature_extractor"])
        subtype_pred_model.load_state_dict(ckpt["subtype_classifier"])
        domain_disc_model.load_state_dict(ckpt["domain_discriminator"])
    ...
    pt_epochs = parameters["num_epochs_pretraining"]
    ad_train_epochs = parameters["num_epochs_adversarial_training"]
    ssl_train_epochs = parameters["num_epochs_semi_supervised_learning"]
    ft_epochs = parameters["num_epochs_fine_tuning"]
    ...
    # ADD #
    row_max_values = X_final_conf_score.max(axis=1)
    X_pred['Confidence'] = row_max_values.tolist()
    #######
